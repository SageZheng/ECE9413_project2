...
class DispatchQueue(object):
    ...
    def runVector(self):
        global cycle
        if self.VectorQueue.qsize()>self.DataQueueSize:
    ...
    def runData(self,nextAvailableCycle):
        ...
        global cycle
        if self.DataQueue.qsize()>=self.DataQueueSize:
...
class ComputeQueue(object):#more like compute union
    ...
    def change(self,mod,lastCycle):
        ...
        if mod==1:
            self.addC=lastCycle+math.ceil(64/self.numLine)+1
            return self.addC+self.add
        elif mod==2:
            self.divC=lastCycle+math.ceil(64/self.numLine)+1
            return self.divC+self.div
        else:
            self.mulC=lastCycle+math.ceil(64/self.numLine)+1
            return self.mulC+self.mul
class MemoryBusyBoard(object):
    ...
    def change(self,add,strid,offset):
        ...
        for i in range(self.numBank):
            if bankChange[choosedBank]:
                self.bank[i]-=(self.depth-1)
            self.bank[i]=max(cycle+1,self.bank[i])        
        for i in range(64):
            choosedBank=(add+offset[i]+strid*i)%self.numBank
        ...
        for i in range(self.numBank):
            if(bankChange[i]):
                self.bank[i]=self.bank[i]+self.depth
